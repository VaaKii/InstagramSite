...
public class AccountController : ControllerBase
{
    ...
    public async Task<ActionResult<JwtResponse>> LogIn([FromBody] Login loginData)
    {
        ...
        var user = await _bll.AppUsers.FirstOrDefaultAsync(appUser.Id);
        appUser.RefreshTokens = user?.RefreshTokens ?? new List<RefreshToken>();

        foreach (var userRefreshToken in appUser.RefreshTokens ?? new List<RefreshToken>())
        {
    ...
    public async Task<ActionResult> RefreshToken([FromBody] RefreshTokenModel refreshTokenModel)
    {
        ...
        // await _bll.AppUsers.FirstOrDefaultAsync().Collection(u => u.RefreshTokens!)
        //     .Query()
        //     .Where(x => 
        ...
        if (refreshToken.Token == refreshTokenModel.RefreshToken)
        {
            ...
            _bll.RefreshTokens.Update(refreshToken);
        }
        var userRoles = await _userManager.GetRolesAsync(appUser);
